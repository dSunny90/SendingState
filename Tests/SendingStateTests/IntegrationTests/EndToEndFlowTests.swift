...
//  SendingState
//
//  Created by SunSoo Jeon on 2023/12/11.
//
...
final class EndToEndFlowTests: XCTestCase {
    ...
    func test_tap_gesture_complete_flow() {
        ...
        // Then
        waitUntil(timeout: 1.0) { [handler] in
            handler.handledActions.count > 0
        }
    ...
    func test_multiple_gesture_triggers() {
        ...
        waitUntil(timeout: 0.5) { [handler] in
            handler.handledActions.count >= 1
        }
        ...
        waitUntil(timeout: 0.5) { [handler] in
            handler.handledActions.count >= 2
        }
