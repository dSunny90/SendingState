...
//  SendingState
//
//  Created by SunSoo Jeon on 18.12.2022.
//
...
final class RealWorldScenariosTests: XCTestCase {
    ...
    /// Scenario: Multiple BindingStores on single view
    func test_multipleBindingStores_onSingleView() {
        // Given
        ...
        let user1 = BindingStore<ProfileView.UserModel, ProfileView>(
            state: ProfileView.UserModel(name: "Sunny", bio: "iOS Developer")
        )
        user1.apply(to: view)
        ...
        let user2 = BindingStore<ProfileView.UserModel, ProfileView>(
            state: ProfileView.UserModel(name: "SunSoo", bio: "iOS Senior Engineer")
        )
        user2.apply(to: view)

        // Then: Assert second user data applied
        XCTAssertEqual(view.userName, "SunSoo")
        XCTAssertEqual(view.userBio, "iOS Senior Engineer")
    }
