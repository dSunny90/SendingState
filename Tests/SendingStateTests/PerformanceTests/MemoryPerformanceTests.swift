...
//  SendingState
//
//  Created by SunSoo Jeon on 08.12.2022.
//
...
final class MemoryPerformanceTests: XCTestCase {
    ...
    // MARK: - BindingStore Memory

    /// Measure AnyBindingStore memory overhead
    func test_anyBindingStore_memory_overhead() {
        measure(metrics: [XCTMemoryMetric()]) {
            var viewModels: [AnyBindingStore] = []

            for i in 0..<1000 {
                let vm = BindingStore<TestConfigurableUIView.Model, TestConfigurableUIView>(
