...
final class NSObjectStateTests: XCTestCase {
    typealias TestStateBindingStore = BindingStore<TestConfigurableUIView.Model, TestStateUIView>
    typealias TestConfigurableBindingStore = BindingStore<TestConfigurableUIView.Model, TestConfigurableUIView>
    ...
    // MARK: - BindingStore.apply(to:) → state + configurer + sender
    ...
    func test_apply_stores_state_and_calls_configurer() {
        ...
        let vm = TestConfigurableBindingStore(state: model)
    ...
    func test_apply_propagates_to_senders() {
        ...
        let vm = TestStateBindingStore(state: model)
    ...
    // MARK: - AnyBindingStore.apply(to:) → observer route

    @MainActor
    func test_anyBindingStore_apply_routes_through_observer() {
        // Given
        let cell = TestConfigurableUIView()
        let model = TestConfigurableUIView.Model(text: "World", value: 13)
        let vm = TestConfigurableBindingStore(state: model)
        let anyBindingStore = vm.eraseToAnyBindingStore()

        // When
        anyBindingStore.apply(to: cell)
    ...
    func test_configure_non_NSObject_calls_configurer_directly() {
        ...
        // so we construct SendingState directly (same as AnyBindingStore does).
